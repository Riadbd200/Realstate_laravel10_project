    ...
    <div class="col-md-2 myUser">
        ...
        <li v-for="(user,index) in users" :key="index"> 
          <a href="" @click.prevent="userMessage(user.id)">
           ...
           <img v-if="user && user.role === 'user' && user.photo" :src="'/upload/user_images/' + user.photo"
              alt="UserImage"
              class="userImg"
            />

            <img v-else-if="user && user.role === 'agent' && user.photo" :src="'/upload/agent/' + user.photo"
              alt="AgentImage"
              class="userImg"
            />
            <span class="username text-center" v-if="user && user.name">{{user.name}}</span>
          </a>
        </li>
        ...
        <div class="card-body chat-msg">
          <ul class="chat" v-for="(msg,index) in allmessages.messages" :key="index">
           ...
           <li class="sender clearfix" v-if="allmessages.user.id === msg.sender_id">
              ...
              <img  :src="'/upload/agent/' + msg.user.photo"
              alt="AgentImage"
              class="userImg"
            />
              </span>
